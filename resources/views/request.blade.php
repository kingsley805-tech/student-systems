      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center justify-start">
          ...
          <ul class="logout-mode bg-white flex items-center justify-center space-x-6  md:ml-96">
            <li class="hover:bg-hovebg rounded-md hover:text-extC ml-4 md:ml-96 p-1 font-semibold text-ray-600"><a href="#">
     ...
     <form action="/req"method='POST'>
      ...
      <div class="dash-content mt-2 py-5   bg-white shadow-md  ">
        <div class="title space-y-3">
            ...
            <span class="inner-name text bg-slate-50 font-extrabold text-textHover m-3 my-10  p-3 rounded-md">Request</span>
        </div>
       
        <div class="reqs shadow-md border border-gray-300 mt-6">
            
    <div class="bg-white p-4 ">
        <h2 class="text-xl sm:text-2xl md:text-5xl font-bold mb-4">Leave Request</h2>
        <p class="text bg-slate-50 font-extrabold text-textHover m-3 my-10  p-3 rounded-md">{{session('mssg')}}</p>
        
        <p class="text  font-extrabold text-red-400 m-3 my-10  p-3 rounded-md">{{session('fail')}}</p>
        
        </div>
        <div class="flex items-center flex-col justify-center ">
         <div class="">
           
        <label for="start"class='text-gray-800 font-bold'>Start Date:</label>
        <input type="date" class="border p-3 rounded-md text-blue-300 m-6 w-32" name='start'>
         </div>
        <div class=""><label for="end"class='text-gray-800 font-bold' >End Date:</label>
          <input type="date"class="border p-3 rounded-md text-blue-300 m-6 w-32" name='end'>
        </div>
      </div>
        <div class="mb-4">
          <label for="reason" class="block text-sm sm:text-base md:text-2xl font-semibold text-gray-700">Reason:</label>
          <textarea id="reason"name='input' class="mt-1 block w-full rounded-md focus:outline-none border-2 border-gray-300 shadow-sm sm:text-sm bg-loghead md:text-2xl py-7" rows="3"></textarea>
        </div>
        <button class="bg-blue-400  hover:bg-loghead hover:text-blue-400 text-white font-semibold py-4 my-8 px-4 rounded-full">
          SUBMIT  REQUEST
        </button>
        <h3 class="mt-4 text-lg sm:text-xl md:text-5xl font-semibold">Recent  Requests <br> <br></h3>
        @foreach ($req as $item)
        
        
<div class="flex items-start gap-2.5">
  <div class="  hover:text-blue-800 text-blue-400 hover:stroke-blue-400 hover:bg-gray-100 dark:hover:bg-gray-700">
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
  ...
  <div class="flex flex-col gap-1 w-full max-w-[320px] ">
     <div class="flex items-center space-x-2 rtl:space-x-reverse">
        ...
        <span class="text-sm font-normal text-gray-500 dark:text-gray-400">{{ session('student') }}</span>
     </div>


     

<textarea disabled id="message" rows="4" class="block p-2.5 w-60 md:w-96 text-md font-bold bg-blue-400 rounded-r-xl rounded-b-xl text-white bg-gray-50  border border-gray-300 ">{{$item->Request_Message}}</textarea>


     


     <span class="text-sm font-normal text-gray-500 dark:text-gray-400">{{session('deliver')}}</span>
  ...
  <div id="dropdownDots" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-40 dark:bg-gray-700 dark:divide-gray-600">
     <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownMenuIconButton">
        ...
        <li>
           <a href="#" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-blue-400">Copy</a>
        </li>
