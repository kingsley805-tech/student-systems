      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center justify-start">
          ...
          <ul class="logout-mode bg-white flex items-center justify-center space-x-6  md:ml-96">
            <li class="hover:bg-h rounded-md hover:text-txtC ml-4 md:ml-96 p-1 font-semibold txt-gray"><a href="#">
                
              <span class="link-name mr-12 hidden md:block text-white"> Settings</span>
          </a></li>
    ...
    <div class="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700 mt-14">
      <div class="dash-content mt-2 py-5   bg-white shadow-md  ">
        <div class="title space-y-3">
            ...
            <span class="inner-name  bg-slate-50 font-extrabold text-textHover m-3   p-3 rounded-md">Exams Report</span>
        </div>
       
        <!--exam result-->
        <div class="result  border border-gray-300 mt-8 space-y-5 shadow-md p-4">
            <div class="text-center font-sans">
                <button class="text-2xl font-bold text-center text-header hover:text-gray-600" id="printBut">PRINT SUBJECT-WISE RESULT</button>
                <div id="report" class="mt-4 hidden md:block">
                  <div class="mt-4"style="margin-top: 1rem;">
                    <p class="text-base font-semibold" style="font-size: 1rem; font-weight: 600;">Rapid School</p>
                    <p class="text-base font-semibold" style="font-size: 1rem; font-weight: 600;">Phone: <span class="font-normal" style="font-weight: normal;">0554440059</span> | <a href="" class="text-blue-400" style="color: #0074D9;">Email: school@rapidentrepreneurs.com</a></p>
                    <p class="text-base font-semibold" style="font-size: 1rem; font-weight: 600;">Address: <span class="font-normal" style="font-weight: normal;">South Suntreso, Kumasi</span></p>
                  </div>
                
                  <h2 class="text-xl font-bold mt-6" style="font-size: 1.5rem; font-weight: bold;">Academic Report</h2>
                
                  <div class="" style="display:flex; margin:0 0 0 16rem;">
                    <div style="margin-right: 55px;">
                      <p class="text-base font-semibold" style="font-size: 1rem; font-weight: 600;">Student Name: <span class="font-normal" style="font-weight: normal;">{{$studentAdd->Name}}</span></p>
                      <p class="text-base font-semibold" style="font-size: 1rem; font-weight: 600;">Student ID: {{$assesment->IndexNumber}}<span class="font-normal" style="font-weight: normal;"></span></p>
                      <p class="text-base" style="font-size: 1rem;">Session:{{$assesment->session}}</p>
                    </div>
                
                    <div style="margin-left: 7rem;" class="ml-60">
                      <p class="text-base font-semibold" style="font-size: 1rem; font-weight: 600;">Class:{{$assesment->class}} <span class="font-normal" style="font-weight: normal;"></span></p>
                   
                      <p class="text-base" style="font-size: 1rem;">Roll Number: {{$studentData->IdentityNumber}}</p>
                  ...
                  <div class="p-4 border border-gray-300 rounded-lg" style="padding: 1rem; border: 1px solid #e2e8f0; border-radius: 0.375rem;">
                    <div class="hidden md:block md:w-full max-w-screen-md mx-auto p-4">
                      <div>
                        <div class="font-semibold">
                          <p>TERM: {{$assesment->term}}</p>
                          <p>YEAR NEXT TERM BEGINS:{{$assesment->year}}</p>
                          <p>DATE: {{$assesment->nextTermBegins}}</p>
                          <p>POSITION:{{$assesment->position}}</p>
                        </div>
                      </div>
                      <table class="min-w-full" style="border-collapse: collapse; width: 100%;">
                        <thead style="background: #2c6ea5;">
                            <tr style="background: #106bb6; color: #fff;border: 1px solid #e5e7eb; border-left:1px solid #e5e7eb ;">
                        ...
                        <tbody style="background: #f3f4f6;">
                            <tr style="border: 1px solid #e5e7eb; border-left:1px solid #e5e7eb ;">
                                <td style="padding: 16px;border: 1px solid #e5e7eb; border-left:1px solid #e5e7eb ;font-weight: 700;">{{$table->subjectTitle}}</td>
                                <td style="padding: 16px;border: 1px solid #e5e7eb; border-left:1px solid #e5e7eb ;">{{$table->classScore}}</td>
                                <td style="padding: 16px;border: 1px solid #e5e7eb; border-left:1px solid #e5e7eb ;">{{$table->examScore}}</td>
                                <td style="padding: 16px;border: 1px solid #e5e7eb; border-left:1px solid #e5e7eb ;">{{$table->Total}}</td>
                                <td style="padding: 16px;border: 1px solid #e5e7eb; border-left:1px solid #e5e7eb ;">{{$table->position}}</td>
                                <td style="padding: 16px;border: 1px solid #e5e7eb; border-left:1px solid #e5e7eb ;">{{$table->remark}}</td>
                            </tr>
                    ...
                    <div class="mt-4 hidden md:block space-y-3 md:flex flex-col md:flex-row space-x-20 items-center justify-center "style='display:flex;flex-direction:rows'>
                      <div class="font-semibold">
                        ...
                        <p>Attendance:{{$assesment->attendance}}</p>
                        <p>Conduct/Character: {{$assesment->conduct}}</p>
                        <p>Out of:.............</p>
                      </div>
                      <div class="font-semibold">
                        <p>Promoted To: {{$assesment->promotedTo}}</p>
                        <p>Attitude: {{$assesment->attitude}}</p>
                        <p>Interest: {{$assesment->interest}}</p>
                        <p>Class Teacher's Remarks:..........</p>
                      </div>
                      <div class="font-semibold">
                    
                        <p>PRINTING COST: {{$assesment->printingCost}}</p>
                        <p>P.T.A. GH¢: {{$assesment->PTAFees}}</p>
                        <p>Sports and Games GH¢: {{$assesment->sportAndGameFees}}</p>
                        <p>Fees for next term:{{$assesment->nextTermFees}}</p>
                ...
                <div class="flex items-center justify-center text-header"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-14 h-14">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M15 15l-6 6m0 0l-6-6m6 6V9a6 6 0 0112 0v3" />
                </svg>
                </div>
                <div class=" flex items-center justify-center font-semibold text-xl hover:text-gray-400 text-header"><button id='download'>Exam: End Of  Semester Examination Report..
                  </button></div>
