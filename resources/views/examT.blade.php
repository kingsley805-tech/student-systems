      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center justify-start">
          ...
          <ul class="logout-mode bg-white flex items-center justify-center space-x-6  md:ml-96">
            <li class="hover:bg-hovrbg rounded-md hover:txt-textC ml-4 md:ml-96 p-1 font-semibold text-ray-600"><a href="#">
    ...
    <div class="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700 mt-14">
      <div class="dash-content mt-2 py-5   bg-white shadow-md  ">
        <div class="title space-y-3">
            <span class="inner-name text bg-slate-50 font-extrabold text-textHover m-3 my-10  p-3 rounded-md">Exam Time Table</span>
        </div>
          <!--exam table-->
        <div class="examt ">
          <div class="bg-white flex flex-col items-center justify-center m-auto pt-4">
            <div class="mx-auto">
                <button class="print font-bold text-3xl text-gray-600 hover:text-hoverText" id="printButton">Print the Exams Table</button>
                <div class="tableDownload hidden md:block bg-white p-4 rounded shadow-lg" id="timetable">
                  @if($examination)
                  <h2 class="text-2xl font-semibold mb-4">Exam: {{$examination->examTitle}} ({{$examination->startDate}} - {{$examination->endDate}})</h2>
              @else
                    <h2 class="text-2xl font-semibold mb-4">Exam details not available</h2>
              @endif
                    @if($class)
                      <h3 class="text-lg font-semibold mb-4">Class: {{$class->name}}</h3>
                    @else
                      <h3 class="text-lg font-semibold mb-4">Class: Class Detail Not Available</h3>

                    @endif
                    <table class="min-w-full" style="border-collapse: collapse; width: 100%;">
                        <thead style="background: #2c6ea5;">
                            <tr style="background: #106bb6; color: #fff;">
                                <th style="padding: 16px; text-align: left;">Subject</th>
                                <th style="padding: 16px; text-align: left;">Paper Code</th>
                                <th style="padding: 16px; text-align: left;">Date</th>
                                <th style="padding: 16px; text-align: left;">Timing</th>
                                <th style="padding: 16px; text-align: left;">Room No.</th>
                            </tr>
                        </thead>
                        <tbody style="background: #f3f4f6;">
                          ...
                          @foreach ($answer as $ans)
                              <tr style="border-bottom: 1px solid #e5e7eb;">
                                  <td style="padding: 16px;">{{$ans->subjectName}}</td>
                                  <td style="padding: 16px;">{{$ans->paperCode}}</td>
                                  <td style="padding: 16px;">{{$ans->paperDate}}</td>
                                  <td style="padding: 16px;">{{$ans->startTime}}</td>
                                  <td style="padding: 16px;">{{$ans->roomNumber}}</td>
                              </tr>
                          @endforeach
                      @else
                          <tr>
        ...
        <div class="block  flex items-center justify-center"><span class="font-bold text-gray-700  ">Click here to download the Examination Table</span>
        </div>
        ...
        <div class="print-download-exam flex items-center justify-center font-semibold text-xl hover:text-gray-400 text-header">
          <a href="#" id="downloadButton">Exam: End Of Third Semester Examination (18-09-2023 - 18-10-2023)</a>
        </div>
